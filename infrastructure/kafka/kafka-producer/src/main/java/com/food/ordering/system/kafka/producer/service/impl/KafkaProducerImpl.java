    ...
    implements KafkaProducer<K, V>
{
    ...
    public KafkaProducerImpl(KafkaTemplate<K, V> kvKafkaTemplate) {
        this.kvKafkaTemplate = kvKafkaTemplate;
    }
    ...
    public void send(String topicName, K key, V message, ListenableFutureCallback<SendResult<K, V>> callback) {
        ...
        try {
            ListenableFuture<SendResult<K,V>> listenableFuture = kvKafkaTemplate.send(topicName,key,message);
    ...
    @PreDestroy
    public void close() {
        if (kvKafkaTemplate != null ) {
            log.info("Closing kafka producer!");
            kvKafkaTemplate.destroy();
        }
    }
}
